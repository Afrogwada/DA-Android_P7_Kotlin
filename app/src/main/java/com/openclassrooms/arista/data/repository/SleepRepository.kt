...
class SleepRepository(
    ...
    fun getAllSleeps(): Flow<List<Sleep>> {
        return sleepDao.getAllSleeps()
            ...
            // Log l'erreur
            .catch { e ->
                Log.e("SleepRepo", "Erreur getAllSleeps", e)
    ...
    suspend fun addSleep(sleep: Sleep) {
        try {sleepDao.insertSleep(sleep.toEntity())}
        catch (e: Exception) {Log.e("SleepRepo", "Erreur addSleep", e)}
    }

    // Supprimer un enregistrement
    suspend fun deleteSleep(sleep: Sleep) {
        // L'ID doit être ajouté au modèle Sleep
        try{sleep.id?.let { sleepDao.deleteSleepById(it)}}
        catch (e: Exception) {Log.e("SleepRepo", "Erreur deleteSleep", e)}
    }
}